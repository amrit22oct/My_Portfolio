...
const Services = () => {
  return (
    ...
    >
      <h1 className="text-[2.8rem] font-bold mb-12 pl-[18px] inline-block border-l-[6px] border-white-400  text-left md:text-[2.4rem] sm:text-[2rem] sm:mb-10">
        My Services
      </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[35px] justify-items-center mt-20 items-stretch">
        {services.map((service, index) => (
          <motion.div
            key={index}
            className="group bg-[rgba(20,20,20,0.9)] p-[35px_25px]  border border-[rgba(0,224,255,0.25)] shadow-[0_0_15px_rgba(0,224,255,0.1)] text-center transition-transform duration-300 will-change-transform hover:-translate-y-1 hover:scale-[1.008] hover:border-[#ff4fff] hover:shadow-[0_0_15px_#00e0ff]"
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: index * 0.15 }}
          >
            <div className="text-[3.2rem] text-[#00ff90] mb-[18px] flex justify-center items-center transition-all duration-300 will-change-transform group-hover:text-[#00e0ff] ">
              {service.icon}
            </div>

            <h2 className="text-[1.4rem] font-semibold mb-[12px] text-[#ff4fff] ">
              {service.title}
            </h2>
